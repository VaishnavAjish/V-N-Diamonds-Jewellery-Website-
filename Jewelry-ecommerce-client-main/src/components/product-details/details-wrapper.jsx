...
const DetailsWrapper = ({ productItem, handleImageActive, activeImg, detailsBottom = false }) => {
  const { sku, img, title, imageURLs, category, description, discount, price, status, reviews, tags, offerDate, parent, children, categoryName } = productItem || {};
  ...
  return (
    <div className="tp-product-details-wrapper">
      <div className="tp-product-details-category">
        <span>{parent || category?.name || categoryName || ''}</span>
      </div>
      ...
      <p>{textMore ? description : `${description ? description.substring(0, 100) : ''}...`}
      ...
      {detailsBottom && <DetailsBottomInfo category={parent || category?.name || categoryName} sku={sku} tag={children || tags?.[1] || tags?.[0]} />}
    </div>
  );
};
