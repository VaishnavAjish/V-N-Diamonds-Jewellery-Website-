...
export default function InvoicesPage() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const data = await apiFetch('/api/invoice/all').catch(() => ({ data: [] }));
  ...
  const toggleInvoiceStatus = async (inv, newStatus) => {
    try {
      const res = await apiFetch(`/api/invoice/${inv.id}`, { method: 'PATCH', body: JSON.stringify({ status: newStatus }) });
      if (res?.success) { setEditInvoice(null); load(); }
    } catch (e) { alert('Error: ' + e.message); }
  };

  const handleDeleteInvoice = async (id) => {
    try {
      const res = await apiFetch(`/api/invoice/${id}`, { method: 'DELETE' });
      if (res?.success) { setDeleteInvoiceId(null); load(); }
    } catch (e) { alert('Error: ' + e.message); }
  };
  ...
  const handleReturnSubmit = async () => {
    ...
    try {
      const res = await apiFetch(`/api/invoice/${returnInvoice.id}/return`, {
        method: 'PATCH',
        body: JSON.stringify({ returnedLineIndexes: returnLineIndexes }),
      });
      if (res?.success) { setReturnInvoice(null); setReturnLineIndexes([]); load(); }
    } catch (e) { alert('Error: ' + e.message); }
  };
  ...
  const handleSave = async (andClose = false) => {
    ...
    try {
      ...
      const url = editingInvoiceId ? `/api/invoice/${editingInvoiceId}` : '/api/invoice/add';
      const method = editingInvoiceId ? 'PATCH' : 'POST';
      const res = await apiFetch(url, { method, body: JSON.stringify(payload) });
      if (res?.success) {
        setSuccessMsg(`Invoice #${res.data?.invoiceNumber || ''} saved successfully!`);
        await load();
      ...
      setErrorMsg(e.message || 'Error saving invoice.');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <>
      <Head><title>Invoices — Harene Diamonds Admin</title></Head>
      <AdminLayout title="Invoices" subtitle={`${invoices.length} total invoices`}>

        {/* Page Header */}
        <div className="page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
          <div>
            <h1 style={{ margin: 0, fontSize: 20, fontWeight: 700 }}>Invoices</h1>
            <p style={{ margin: 0, color: '#64748b', fontSize: 13 }}>Invoices created directly or converted from memos</p>
          </div>
          <button
          ...
          >
            {showForm ? '✕ Cancel' : editingInvoiceId ? '✏️ Edit Invoice' : '+ New Invoice'}
          </button>
        </div>
                  ...
                  <table style={{ minWidth: 800, width: 'max-content', borderCollapse: 'collapse', fontSize: 11 }}>
                    ...
                    <tbody>
                      <tr style={{ borderBottom: '1px solid #f1f5f9', background: '#eff6ff' }}>
                        ...
                        <td style={tdStyle}><input type="date" defaultValue={new Date(Date.now()+4*24*3600*1000).toISOString().split('T')[0]} style={{ ...lineInputStyle, width: 110 }} /></td>
                ...
                {invoices.map((inv) => {
                  ...
                  return (
                    <tr key={inv.id}>
                      ...
                      <td><span className={`badge ${inv.status === 'paid' ? 'badge-green' : 'badge-yellow'}`}>{inv.status === 'paid' ? 'Invoice' : 'Memo'}</span></td>
                      <td className="text-muted text-sm">{new Date(inv.createdAt).toLocaleDateString()}</td>
                      <td>
                        <div style={{ display: 'flex', gap: 6 }}>
                          <button className="btn btn-sm" onClick={() => handleEditInvoice(inv)}>✏️ Edit</button>
                          <button className="btn btn-sm" onClick={() => setEditInvoice(inv)}>🔄 Status</button>
                          <button className="btn btn-sm" disabled={lineCount === 0} onClick={() => { setReturnInvoice(inv); setReturnLineIndexes([]); }}>↩️ Return</button>
                          <button className="btn btn-sm btn-danger" onClick={() => setDeleteInvoiceId(inv.id)}>🗑️</button>
                        </div>
                      </td>
                    </tr>
                  );
          ...
          <div className="modal-overlay">
            <div className="modal">
              <div className="modal-title">
                <span>✏️ Edit Invoice #{editInvoice.invoiceNumber}</span>
                <button className="btn btn-sm" onClick={() => setEditInvoice(null)}>✕</button>
              </div>
              <p style={{ color: 'var(--text-secondary)', fontSize: 13 }}>
                <strong>Invoice</strong> = paid & finalized. <strong>Memo</strong> = unpaid, items can still be returned.
              </p>
              <div style={{ display: 'flex', gap: 10 }}>
                <button
                  className="btn"
                  style={editInvoice.status !== 'paid' ? { background: '#f59e0b', color: '#fff' } : undefined}
                  onClick={() => toggleInvoiceStatus(editInvoice, 'unpaid')}
                >
                  📋 Memo (Unpaid)
                </button>
                <button
                  className="btn"
                  style={editInvoice.status === 'paid' ? { background: '#16a34a', color: '#fff' } : undefined}
                  onClick={() => toggleInvoiceStatus(editInvoice, 'paid')}
                >
                  🧾 Invoice (Paid)
                </button>
              </div>
              <div className="modal-actions">
                <button className="btn" onClick={() => setEditInvoice(null)}>Close</button>
              </div>
            </div>
          </div>
        )}
          ...
          <div className="modal-overlay">
            <div className="modal">
              <div className="modal-title">
                ...
                <button className="btn btn-sm" onClick={() => setDeleteInvoiceId(null)}>✕</button>
              </div>
              <p style={{ color: 'var(--text-secondary)', fontSize: 13 }}>
                Delete this invoice? If it came from a memo, the memo will reopen in the Memos list.
              </p>
              <div className="modal-actions">
                <button className="btn" onClick={() => setDeleteInvoiceId(null)}>Cancel</button>
                <button className="btn btn-danger" onClick={() => handleDeleteInvoice(deleteInvoiceId)}>Delete</button>
              </div>
            </div>
          </div>
        )}

        {/* Memo/Invoice return modal */}
        {returnInvoice && (
          <div className="modal-overlay">
            <div className="modal">
              <div className="modal-title">
                <span>↩️ Return Items — #{returnInvoice.invoiceNumber}</span>
                <button className="btn btn-sm" onClick={() => setReturnInvoice(null)}>✕</button>
              </div>
              ...
              {returnInvoice.items.filter((i) => !i.__header__).map((item, idx) => (
              ...
              <div className="modal-actions">
                <button className="btn" onClick={() => setReturnInvoice(null)}>Cancel</button>
