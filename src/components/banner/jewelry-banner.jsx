...
const JewelryBanner = ({ settings }) => {
  ...
  const dynamic_slider_data = settings?.heroBanners?.length > 0
    ? settings.heroBanners
    : fallback_slider_data;
        ...
        >
          {dynamic_slider_data.map((item, i) => (
            <div
              ...
              style={settings?.customBackgroundColor ? { backgroundColor: settings.customBackgroundColor } : {}}
            >
              <div className="tp-slider-thumb-4">
                {typeof item.img === 'string' ? (
                  <img src={item.img} alt="slider img" className="main-slider-img" style={{ width: item.imageWidth ? `${item.imageWidth}px` : '700px' }} />
                ) : (
              ...
              <div className="tp-slider-video-wrapper">
                ...
                <div className={`tp-slider-video transition-3 ${play ? 'full-width' : ''}`}>
                  ...
                  <video loop>
                    <source type="video/mp4" src="http://weblearnbd.net/tphtml/videos/V N Diamonds/jewellery-1.mp4#t=3" />
                  </video>
                </div>
                {/* <!-- video button --> */}
              ...
              {dynamic_slider_data.map((item, i) => (
                <div key={i} className="tp-slider-nav-item d-flex align-items-center" style={{ padding: '8px 0' }}>
                  <div className="tp-slider-nav-icon" style={{ width: '40px', flexShrink: 0, display: 'flex', justifyContent: 'center' }}>
                    <span>
                      {typeof item.navIcon === 'string' ? <img src={item.navIcon} alt="icon" style={{ maxWidth: '40px', filter: 'brightness(0) invert(1)' }} /> : <Image src={item.navIcon || slider_nav_data[i]?.icon} alt="icon" style={{ maxWidth: '40px', filter: 'brightness(0) invert(1)' }} />}
                    </span>
                  </div>
