...
const JewelryBrands = ({ settings }) => {
  const brands = settings?.brands?.length > 0
    ? settings.brands
    : brand_data;
                ...
                <Swiper {...slider_setting} modules={[Navigation]} className="tp-brand-slider-active swiper-container">
                  {brands.map((logo, i) => (
                    <SwiperSlide key={i} className="tp-brand-item text-center">
                      <a href="#">
                        {typeof logo === 'string' ? <img src={getImgSrc(logo)} alt="brand img" style={{maxWidth: '100px'}}/> : <Image src={logo} alt="brand img" />}
                      </a>
                    </SwiperSlide>
                  ))}
                </Swiper>
