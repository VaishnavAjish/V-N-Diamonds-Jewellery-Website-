...
const instagram_data = [
  ...
  { id: 2, link: 'https://www.instagram.com/harenejewels?utm_source=ig_web_button_share_sheet&igsh=ZDNlZDc0MzIxNw==', img: insta_3 },
  { id: 3, link: 'https://www.instagram.com/harenejewels?utm_source=ig_web_button_share_sheet&igsh=ZDNlZDc0MzIxNw==', img: insta_4 },
  { id: 4, link: 'https://www.instagram.com/harenejewels?utm_source=ig_web_button_share_sheet&igsh=ZDNlZDc0MzIxNw==', img: insta_6 },
]
const MobileMenus = () => {
  ...
  if (!isLoading && !isError && products?.data?.length > 0) {
    ...
    content = (
      <div className="row">
        {product_items.slice(0, 4).map((item, i) => (
          <div key={item?._id || item?.id || i} className="col-md-3">
      ...
      <nav className="tp-main-menu-content">
        {mobile_menu.map((menu, i) => {
          if (menu.adminOnly) {
            ...
            if (role !== 'admin' && role !== 'superadmin' && role !== 'manager' && role !== 'manger') {
              return null;
            }
            return (
              <ul key={menu.id}>
                <li>
                  <a href={`http://localhost:4000/auth-redirect?token=${accessToken}&user=${encodeURIComponent(JSON.stringify(userInfo))}`} target="_blank" rel="noopener noreferrer">
                    {menu.title}
                  </a>
                </li>
              </ul>
