...
const ColorFilter = ({setCurrPage,shop_right=false}) => {
  ...
  const handleColor = (clr) => {
    setCurrPage(1)
    const slug = clr.toLowerCase().replace("&", "").split(" ").join("-");
    const newQuery = { ...router.query, color: slug };
  ...
  if (!isLoading && !isError && filtersData?.data?.color?.length === 0) {
    content = <ErrorMsg msg="No Colors found!" />;
  }
  if (!isLoading && !isError && filtersData?.data?.color?.length > 0) {
    uniqueColors = filtersData.data.color.filter(c => c && c.trim() !== "");
    
    const visibleColors = showAll ? uniqueColors : uniqueColors.slice(0, 5);
    
    content = visibleColors.map((item, i) => {
      if (item) {
        const isActive = router.query.color === item.toLowerCase().replace("&", "").split(" ").join("-");
        return (
          <button
            key={i}
  ...
  return (
    <>
      <div className="tp-shop-widget mb-50">
        <h3 className="tp-shop-widget-title" style={{ fontSize: '11px', color: '#888', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '15px' }}>Color</h3>
        <div className="tp-shop-widget-content">
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
            ...
            {uniqueColors.length > 5 && (
              ...
              >
                {showAll ? "− Show Less" : `+ ${uniqueColors.length - 5} More`}
              </button>
            )}
          </div>
        </div>
