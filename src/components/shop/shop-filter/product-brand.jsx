...
const ProductBrand = ({setCurrPage,shop_right=false}) => {
  ...
  const handleBrandRoute = (brand) => {
    ...
    const newQuery = { ...router.query, brand: slug };
  ...
  if (isLoading) {
    ...
    content = brand_items.map((b, i) => (
      <div key={b?._id || b?.id || i} className="tp-shop-widget-brand-item">
        <a
          onClick={() => handleBrandRoute(b.name)}
          style={{ cursor: "pointer" }}
        >
          <Image src={b.logo} alt="brand" width={60} height={50} unoptimized={true} />
        </a>
      </div>
    ));
  }
