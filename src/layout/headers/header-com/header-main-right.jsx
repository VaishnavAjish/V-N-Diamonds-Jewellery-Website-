...
const HeaderMainRight = ({ setIsCanvasOpen }) => {
  ...
  return (
    <div className="tp-header-main-right d-flex align-items-center justify-content-end">
      ...
      >
          {userInfo?.imageURL ? (
            <a onClick={() => setShowLogoutConfirm(false)} className="cursor-pointer" style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: '40px', height: '40px', borderRadius: '50%', overflow: 'hidden', border: '1px solid currentColor' }}>
              <Image
            ...
            <a onClick={() => setShowLogoutConfirm(false)} className="cursor-pointer" style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: '40px', height: '40px', borderRadius: '50%', border: '1px solid currentColor', color: 'inherit', textDecoration: 'none' }}>
              <span className="text-uppercase" style={{ fontSize: '18px', fontWeight: 600 }}>
                {userInfo?.name[0]}
              </span>
            </a>
          ) : (
            <Link href="/login" style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: '40px', height: '40px', borderRadius: '50%', border: '1px solid currentColor', color: 'inherit', textDecoration: 'none' }}>
              <User />
            </Link>
          )}
