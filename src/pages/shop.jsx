  ...
  if (!isLoading && !isError && products?.data?.length > 0) {
    ...
    if (query.certificate) {
      product_items = product_items.filter((p) => {
        ...
        const flatCert = certObj.value.toLowerCase().replace(/[^a-z0-9]/g, "");
        const flatTarget = query.certificate.toLowerCase().replace(/[^a-z0-9]/g, "");
        
        return flatCert === flatTarget || flatCert.includes(flatTarget) || flatTarget.includes(flatCert);
      });
    }
